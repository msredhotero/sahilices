...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 02-04-2019 a las 17:51:00
...
CREATE TABLE `dbcotizaciondetallesaux` (
...
INSERT INTO `dbcotizaciones` (`idcotizacion`, `refclientes`, `refestados`, `refestadocotizacion`, `refcontactos`, `refmotivosoportunidades`, `reftipostrabajos`, `refusuarios`, `observaciones`, `fechacrea`, `fechamodi`, `usuariomodi`, `refempresas`) VALUES
(1, 5, 1, 2, 1, 0, 0, 2, NULL, '2019-04-02 00:00:00', '2019-04-02 00:00:00', '', NULL);
...
(1, 'YPF', 'Juan Carlos Tato', '0221 6184415', 'aranzazu@aif.org.ar', '', 4, 4, '', 1, 1, 1, 0, 3, '2019-04-01 04:38:47');
...
CREATE TABLE `dbtipotrabajoconceptos` (
...
ALTER TABLE `dbconceptos`
  ...
  ADD KEY `fk_c_tc_idx` (`reftipoconceptos`);
...
ALTER TABLE `dbcotizaciones`
  ...
  ADD KEY `fk_dbcotizaciones_tbestadocotizacion_idx` (`refestadocotizacion`) USING BTREE;
...
ALTER TABLE `dbtipotrabajoconceptos`
  ADD PRIMARY KEY (`idtipotrabajoconcepto`),
  ADD KEY `fk_tc_t_idx` (`reftipostrabajos`),
  ADD KEY `fk_tc_c_idx` (`refconceptos`);
...
--
ALTER TABLE `dbcotizaciondetallesaux`
  MODIFY `idcotizaciondetalleaux` int(11) NOT NULL AUTO_INCREMENT;
